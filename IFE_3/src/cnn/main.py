...
def run_nn(cfg, mode, model, loader, criterion=None, optim=None, scheduler=None, apex=None, writer_dict=None):
    ...
    if mode in ['valid', 'test'] and cfg.gen_feat:
        net1=nn.Sequential(model[0],*list(model[1].children())[:-2])
        net2=nn.Sequential(model[0],*list(model[1].children())[:-1])
    ...
    for i, (inputs, targets, ids) in enumerate(loader):
        ...
        if mode in ['valid', 'test'] and cfg.gen_feat:
            #get 1-D feature
            output_features=net2(inputs)[:,:,0,0]
            # get 3-D feature
            outputs_3D = net1(inputs)
            pretrained_dict = model.state_dict()
            #embed()
            fc_mat = pretrained_dict['1.fc.weight']
        ...
        with torch.no_grad():
            ...
            if mode in ['valid', 'test'] and cfg.gen_feat:
                ...
                for batch_id in range(batch_size):
                    outputs_3D_mat = outputs_3D[batch_id,:,:,:].view(2048,14*14)
                    output_features_3D.extend([fc_mat.mm(outputs_3D_mat).view(6,14,14).cpu().numpy()])
