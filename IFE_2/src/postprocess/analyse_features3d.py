...
parser.add_argument('--output3d', default='./test3d')
...
def main():
    ...
    if args.istest:
        ...
        output3d = os.path.join(args.output3d, 'fold{}_test_{}'.format(args.fold,args.ttaid))
        annotations_pkl  = cfg.data.test.annotations
    else:
        output = os.path.join(args.output, 'fold{}_train'.format(args.fold))
        output3d = os.path.join(args.output3d, 'fold{}_train'.format(args.fold))
        annotations_pkl = cfg.data.train.annotations

    if not os.path.exists(output):
        os.mkdir(output)
    if not os.path.exists(output3d):
        os.mkdir(output3d)
    ...
    for idx in range(len(df)):
        ...
        if list_ID[0] in imgs:
            image = np.array([[0.0] * (1280 + 6)]*60)
            image3d = np.zeros([60,16,16,6])
            for i, ID in enumerate(list_ID):
                data = imgs[ID]
                image[i,:] = np.concatenate([data['feature'], data['pred']])
                image3d[i,:] = data['feature_3D'].transpose(1,2,0)

            np.save(output + '/' + list_ID[0] + '.npy', image)
            np.save(output3d + '/' + list_ID[0] + '.npy', image3d)
