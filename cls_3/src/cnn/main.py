...
def run_nn(cfg, mode, model, loader, criterion=None, optim=None, scheduler=None, apex=None, writer_dict=None):
    ...
    for i, (inputs, inputs3d, targets, ids) in enumerate(loader):

        batch_size = len(inputs3d)

        inputs_ = inputs.cuda()
        inputs3d_ = inputs3d.cuda()
        targets_ = targets.cuda()
        #embed()
        outputs_ = model(inputs_,inputs3d_)
        ...
        if mode in ['train']:
            if (i+1) % cfg.log_size == 0:
                total_step = min(len(loader), (cfg.epoch_size + cfg.epoch_size_precisebn))
                eta = int(elapsed / (i+1) * (total_step-(i+1)))
                progress = f'\r[{mode}] {i+1}/({total_step}) {elapsed}(s) eta:{eta}(s) loss:{np.mean(losses):.6f} lr:{util.get_lr(optim):.2e}'
